    ...
    return (
        <section className='ourprocess_bg pt-[50px]'>
            <div className="w-[90%] px-[20px] mx-auto 2xl:py-100px] md:py-[70px] py-[50px] " id="our-process">
                <h2 className="whitespace-nowrap text-left font-[genkaimincho] font-[600] text-[50px] text-black uppercase pb-[5px] ">Our Process</h2>
                <div className="h-[5px] w-[150px] mb-[10px] md:mb-[20px] bg-[#7a1a4a] rounded-md"></div>
                <div className="lg:grid lg:grid-cols-2 gap-10 xl:gap-14 py-[10px] md:py-[20px] justify-between flex flex-col">
                    <div className="">
                        <ol className="list-inside list-none space-y-6">
                            {processSteps1.map((step, index) => (
                                <li key={step.number} className="">
                                    <span className='font-bold  text-[30px] pr-2'>{index + 1}. </span>
                                    <span className={`mr-2 font-bold text-[24px] 2xl:text-[30px] text-left font-[segoe-ui] text-black whitespace-nowrap`}>
                                        {step.number}
                                    </span>
                                    <span className='text-[22px] 2xl:text-[30px]'>{step.description}</span>
                                </li>
                            ))}
                        </ol>
                    </div>
                    <div className="">
                        <ol className="list-inside list-none space-y-6 ">
                            {processSteps2.map((step, index) => (
                                <li key={step.number} className="">
                                    <span className='font-bold  text-[30px] pr-2'>{index + 4}. </span>
                                    <span className={`mr-2 font-bold text-[24px] 2xl:text-[30px] text-left font-[segoe-ui] text-black whitespace-nowrap`}>
                                        {step.number}
                                    </span>
                                    <span className='text-[22px] 2xl:text-[30px]'>{step.description}</span>
